...
- Story 30: a country/language filter dimension is dropped. A difficulty-generated set defaults to international scope instead, using Wikidata's sitelinks count (already validated during the metadata-sourcing spike) as the signal. Playlist selection is exactly two modes, Difficulty-Based (Auto-Generated) and Custom (a playlist the player already has access to, or one pasted directly); theme-request generation and the owned/member/published-public distinction are both dropped, see the 2026-09 `DECISIONS.md` entry.
