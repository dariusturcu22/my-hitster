...
- How much typo tolerance an in-round artist/title guess gets before counting as correct (see `GAME_DESIGN.md`'s Earning tokens section) is a real edit-distance/fuzzy-matching problem, not machine learning, the mechanism is decided; the actual threshold is still being calibrated against example guesses, in progress, continuing in a future session, see the worked examples note below. Distinct from story 18's song-verification criteria, this is about matching a player's guess text during a round, not about trusting a submitted song's metadata.
- Typo-tolerance calibration in progress, first pass, not final: "Bohemian Rapsody" for "Bohemian Rhapsody" (one letter dropped) counts as correct; "Bohemian" alone (half the title missing) does not; "Beatles" for "The Beatles" (article dropped) does not; "Guns N Roses" for "Guns N' Roses" (apostrophe dropped) counts as correct, "Guns and Roses" (spelled out) undecided; "Never Gonna Give You App" for "...Up" (one real word swapped for an unrelated one, small edit distance) does not count, a case worth remembering since it shows edit distance alone can't be the whole rule. More examples needed before a real threshold can be set.
